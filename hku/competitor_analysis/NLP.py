...
farfetch_df_clean.rename(columns={'pro_desc_eng': 'prod_desc_eng'})
...
# MAGIC %md
# MAGIC
# MAGIC - Combine the attributes: *color_desc, long_desc*

# COMMAND ----------

lc_df_clean['concat_text'] = lc_df_clean['prod_desc_eng'].str.cat([lc_df_clean.brand_desc,lc_df_clean.compost,lc_df_clean.color_desc,lc_df_clean.long_desc],sep=' ')

lc_df_clean.head()

# COMMAND ----------

farfetch_df_clean['concat_text'] = farfetch_df_clean['pro_desc_eng'].str.cat([farfetch_df_clean.brand_desc,farfetch_df_clean.compost,farfetch_df_clean.color_desc,farfetch_df_clean.long_desc],sep=' ')
...
lc_df_processed['text_token'] = lc_df_processed['concat_text'].apply(lambda X: text_prepocessing(X))
lc_df_processed['text_processed'] = lc_df_processed['text_token'].apply(lambda x: ' '.join(x))
...
farfetch_df_processed['text_token'] = farfetch_df_processed['concat_text'].apply(lambda X: text_prepocessing(X))
farfetch_df_processed['text_processed'] = farfetch_df_processed['text_token'].apply(lambda x: ' '.join(x))
...
farfetch_df_processed['word_vector'] = farfetch_df_processed['text_processed'].apply(lambda x: get_vector(x))

farfetch_df_processed.head()

# COMMAND ----------

lc_df_processed['word_vector'] = lc_df_processed['text_processed'].apply(lambda x: get_vector(x))
...
lc_df_result = lc_df_processed[['atg_code', 'word_vector']]
farfetch_df_result = farfetch_df_processed[['farfetch_id', 'word_vector']]
...
lc_df_result.to_csv("/dbfs/nlp/lc_result.csv", index = False)
dbutils.fs.cp("nlp", os.path.join(team_path, "nlp"), recurse=True)

# COMMAND ----------

dbutils.fs.mkdirs("nlp")
lc_df_result.to_json("/dbfs/nlp/lc_result.json")
dbutils.fs.cp("nlp", os.path.join(team_path, "nlp"), recurse=True)

# COMMAND ----------

dbutils.fs.mkdirs("nlp")
farfetch_df_result.to_csv("/dbfs/nlp/farfetch_result.csv", index = False)
dbutils.fs.cp("nlp", os.path.join(team_path, "nlp"), recurse=True)

# COMMAND ----------

dbutils.fs.mkdirs("nlp")
farfetch_df_result.to_json("/dbfs/nlp/farfetch_result.json")
dbutils.fs.cp("nlp", os.path.join(team_path, "nlp"), recurse=True)
