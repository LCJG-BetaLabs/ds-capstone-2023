...
smote_df_with_vectors_train_resnet = spark.read.parquet(os.path.join(team_path, "smote_df_with_vectors_train_resnet.parquet"))
smote_df_with_vectors_train_inception = spark.read.parquet(os.path.join(team_path, "smote_df_with_vectors_train_inception.parquet"))
smote_df_with_vectors_train_efficientnet = spark.read.parquet(os.path.join(team_path, "smote_df_with_vectors_train_efficientnet.parquet"))
...
layers = [len(smote_df_with_vectors_train_resnet.select("features").first()[0]), 128, 128, 7]
...
mlp_rn = MultilayerPerceptronClassifier(layers=layers, labelCol='label', featuresCol='features', maxIter=100, 
                                      blockSize=128, seed=1234, solver="l-bfgs", stepSize=0.05, tol=1e-05, 
                                      initialWeights=None)

# 训练模型
model_mlp_rn = mlp_rn.fit(smote_df_with_vectors_train_resnet)
...
layers = [len(smote_df_with_vectors_train_inception.select("features").first()[0]), 128, 128, 7]
...
# 训练模型
model_mlp = mlp.fit(smote_df_with_vectors_train_inception)


# COMMAND ----------

featuresTestDF_en = test_ic
...
layers = [len(smote_df_with_vectors_train_efficientnet.select("features").first()[0]), 128, 128, 7]
...
# 训练模型
model_mlp_en = mlp_en.fit(smote_df_with_vectors_train_efficientnet)
...
layers = [len(smote_df_with_vectors_train_resnet.select("features").first()[0]), 126, 126, 7]

# 创建MultilayerPerceptronClassifier分类器，设置层数、标签列、特征列、最大迭代次数和添加softmax层
mlp_rn= MultilayerPerceptronClassifier(layers=layers, labelCol='label', featuresCol='features', maxIter=100, 
                                      blockSize=126, seed=0, solver="l-bfgs", stepSize=0.05, tol=1e-05, 
                                      initialWeights=None)
# 训练模型
model_mlp_rn = mlp_rn.fit(smote_df_with_vectors_train_resnet)
