...
df_embs_original = pd.read_csv(os.path.join("file:/dbfs", "image_embedding_data.csv"))
...
df_embs_oversample = df_embs_train_undersample.loc[df_embs_train_undersample['Y']==4]
...
df_embs_underample4 = df_embs_train_undersample.loc[df_embs_train_undersample['Y']==5]
df_embs_underample5 = df_embs_train_undersample.loc[df_embs_train_undersample['Y']==6]

for i in range(2, 6): # group 1 is not resampled
    exec(f"df_embs_resample{i} = resample(df_embs_underample{i}, replace=True, n_samples=df_embs_oversample.shape[0], random_state=123)")

df_embs_train = pd.concat((df_embs_oversample, df_embs_underample1, df_embs_resample2, df_embs_resample3, df_embs_resample4, df_embs_resample5))
...
# Train the model
history = model.fit(X_train, y_train, batch_size=16, epochs=EPOCH_SIZE, verbose=1, validation_data=(X_test, y_test),callbacks=[check_point])
...
# text data
X7 = df_cleansed[df_cleansed['class']==7]['comc_long_desc'].tolist()
...
df_embs_class7 = pd.concat([df_embs_class7.iloc[:, :-2], df_embs_text, df_embs_class7.iloc[:, -2:]], axis=1)
